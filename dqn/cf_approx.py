...
class CellDb:
    ...
    def __init__(self, owner, curr_time):
        self.x = None
        self.y = None
        self.owner = owner
    ...
    def get_take_time_raw(self, time_diff):
        if time_diff <= 0:
            return 33
        return 30 * (2 ** (-time_diff / 30.0)) + 3

    def get_take_time(self, curr_time):
        if not self.is_taking:
            if self.owner != 0:
                take_time = self.get_take_time_raw(curr_time - self.occupy_time)
            else:
                take_time = 2
        else:
            take_time = -1
    ...
    def to_dict(self, curr_time):
        return {
            ...
            't': self.get_take_time(curr_time),
    ...
    def refresh_build(self, currTime):
    ...
    def Attack(self, user, currTime, boost=False, adjCells=0):
        ...
        takeTime = (self.get_take_time(currTime) * min(1, 1 - 0.25 * (adjCells - 1))) / (1 + user.energy / 200.0)
...
def UpdateGame(currTime, timeDiff):
    ...
    for cell in cells:
        if cell.refresh_build(currTime):
            if cell.owner not in dirtyUserIds:
                dirtyUserIds[cell.owner] = set()
            dirtyUserIds[cell.owner].add('base')
...
def ClearGame(currTime, softRestart, gameSize, gameId):
    ...
    if redisConn:
        ...
        info = InfoDb.query.get(0)
        redisConn.set('gameInfo', json.dumps(info.to_dict(currTime)))
        db.session.commit()
...
def GetGameInfo():
    ...
    if redisConn:
        ...
        retInfo['info'] = {'width': info.width, 'height': info.height, 'time': currTime, 'end_time': info.end_time,
                           'join_end_time': info.join_end_time, 'game_id': info.game_id, 'game_version': GAME_VERSION,
    ...
    if refreshGame:
        ...
        userInfo = []
        for user in users:
            userInfo.append(user.to_dict(useSimpleDict))
        db.session.commit()
    ...
    for c in changedCells:
        retCells.append(c.to_dict(currTime))
